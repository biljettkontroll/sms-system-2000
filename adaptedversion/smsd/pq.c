...
GNOKII_API gint DB_InsertSMS (const gn_sms * const data, const gchar * const phone)
{
  ...
  g_string_printf (buf, "INSERT INTO %s.%s_inbox (\"number\", \"smsdate\", \"insertdate\",\
                         \"text\", \"processed\") VALUES ('%s', \
                         '%02d-%02d-%02d %02d:%02d:%02d+01', 'now', '%s', 0)",
                   schema, prefix,
                    data->remote.number,
...
GNOKII_API void DB_Look (const gchar * const phone)
{
  ...
  g_string_printf (buf, "SELECT id, number, text, dreport FROM %s.%s_outbox \
                         WHERE processed=0 AND localtime(0) >= not_before \
                         AND localtime(0) <= not_after %s ORDER BY priority DESC FOR UPDATE",
                   schema, prefix, phnStr->str);
  g_string_free (phnStr, TRUE);
  ...
  for (i = 0; i < PQntuples (res1); i++)
  {
    ...
    g_string_printf (buf, "UPDATE %s.%s_outbox SET processed=1, error='%d', \
                           processed_date='now' WHERE id='%s'",
                     schema, prefix, error, PQgetvalue (res1, i, 0));
