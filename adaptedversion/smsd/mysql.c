...
*/
  ...
  { // Multipart Message !
    ...
    buf = g_string_sized_new (256);
                    g_string_printf (buf, "SELECT count(id) FROM %s_multipartinbox \
    ...
    g_string_printf (buf, "INSERT INTO %s_multipartinbox (number, smsdate, \
    ...
    if (mysql_real_query (&mysqlIn, buf->str, buf->len))
    {
      gn_log_xdebug ("%d: INSERT INTO %s_multipartinbox failed.\n", __LINE__, prefix);
      gn_log_xdebug ("%s\n", buf);      
      gn_log_xdebug ("Error: %s\n", mysql_error (&mysqlIn));
      g_string_free (buf, TRUE);
      return SMSD_NOK;
    ...
    g_string_printf (buf, "SELECT count(DISTINCT text) FROM %s_multipartinbox \
                           WHERE number='%s' AND refnum=%i AND maxnum=%i and processed = 0" ,
                     prefix, data->remote.number,
                     data->udh.udh[0].u.concatenated_short_message.reference_number,
                     data->udh.udh[0].u.concatenated_short_message.maximum_number);
    ...
    {
      gchar *tmpText;
      g_string_printf (buf, "SELECT DISTINCT text FROM %s_multipartinbox \
      ...
      g_free(tmpText);
      g_string_printf(buf, "UPDATE %s_multipartinbox SET processed = 1 \
  ...
  /* Detect duplicates */
  g_string_printf (buf, "SELECT count(id) FROM %s_inbox \
                         WHERE text = '%s' AND smsdate = '%04d-%02d-%02d %02d:%02d:%02d'",
		   prefix, text, data->smsc_time.year, data->smsc_time.month,
		   data->smsc_time.day, data->smsc_time.hour,
		   data->smsc_time.minute, data->smsc_time.second);
  if (mysql_real_query (&mysqlIn, buf->str, buf->len))
  {
  ...
  g_string_printf (buf, "INSERT INTO %s_inbox (number, smsdate, \
                          text, %s processed) VALUES ('%s', \
                          '%04d-%02d-%02d %02d:%02d:%02d', '%s', %s '0')",
                   prefix, phone[0] != '\0' ? "phone," : "", data->remote.number,
                   data->smsc_time.year, data->smsc_time.month,
                   data->smsc_time.day, data->smsc_time.hour,
...
GNOKII_API void DB_Look (const gchar * const phone)
{
  ...
  g_string_printf (buf, "SELECT id, number, text, dreport FROM %s_outbox \
                         WHERE processed='0' AND CURTIME() >= not_before \
                         AND CURTIME() <= not_after %s", prefix, phnStr->str);
  g_string_free (phnStr, TRUE);
  ...
  while ((row = mysql_fetch_row (res1)))
  {
    ...
    g_string_printf (buf, "UPDATE %s_outbox SET processed='1', error='%d', \
