...
//  smsdConfig.smsSets = 0;
  ...
  while (1)
  {
    ...
    c = getopt_long (argc, argv, "u:p:d:c:s:m:l:f:t:vi:S:n:q:b:0h", longOptions, &optionIndex);
    if (c == EOF)
      break;
    switch (c)
    {
     ...
     case 'n':
        g_free(connection.prefix);
        connection.prefix = g_strdup (optarg);
        memset (optarg,'x', strlen(optarg));
        break;
 
     case 'q':
        smsdConfig.checkForSms = atoi (optarg);
        break;
...
static void *SendSMS (void *a)
{
  ...
  while (1)
  {
    ...
    gn_log_xdebug ("SendSMS usleep 50 000.\n");
    usleep (500000);
  }
}
